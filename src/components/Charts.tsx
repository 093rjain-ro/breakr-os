...
import { StockData, Recommendation, CryptoAssetWithRecommendation } from '../types';
...
interface CryptoChartProps {
    asset: CryptoAssetWithRecommendation;
}

export const CryptoPriceChart: React.FC<CryptoChartProps> = ({ asset }) => {
    const data = useMemo(() => {
        // Simulate 24h data points for visualization
        return Array.from({ length: 24 }).map((_, i) => {
            const isNow = i === 23;
            // Create a somewhat realistic curve ending at current LTP
            const volatility = 0.02; // 2% volatility
            const randomChange = 1 + (Math.random() * volatility * 2 - volatility);
            const price = isNow ? asset.ltp : asset.ltp * randomChange;
        ...
        <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data}>
                    ...
                    />
                    <Area type="monotone" dataKey="price" stroke="#f97316" fill="url(#colorCrypto)" strokeWidth={2} isAnimationActive={false} />
                </AreaChart>
            </ResponsiveContainer>
        </div>
    );
